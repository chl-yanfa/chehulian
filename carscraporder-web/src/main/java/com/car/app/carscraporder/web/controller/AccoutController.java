...
public class AccoutController {
    ...
    private RedisService redisService;
	    ...
	    public ResultBean<Map<String, Object>> doLogin(@RequestParam("username") String username,
	        ...
	        if (user != null) {
	        	// 登录成功
	              ...
	              map.put("certificationState", user.getCertificationState());
	    ...
	    @ResponseBody
	    public ResultBean<Boolean> doLogin(HttpServletResponse response) {
	    	
	    	//扫码后获取微信号
	    	String weixincode = null;
	    	//1.根据微信号查询sys_client表是否存在这样的数据
			
	       
	    	//2.存在认证成功存入缓存生产ticket
	    	
	    	//3.不存在保存这样一个用户认证成功存入缓存生产ticket
	...
	@ResponseBody
	public ResultBean<Boolean> doRegister(ClientVO clientVO)
			throws Exception {
